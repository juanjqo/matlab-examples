...
try           
    xbase = cs.get_object_pose(jointnames{1});
    robot.set_reference_frame(xbase);
    robot.set_base_frame(xbase);
    cs.set_object_pose('base', xbase);
    %----------------------------------------------------------------
    ...
    xplane = cs.get_object_pose('target_plane');
    ...
    %  Plane constraints
    safe_distance = 0.1;
    wall_plane  = Adsharp(cs.get_object_pose('wall_plane'),  k_); 
    floor_plane = Adsharp(cs.get_object_pose('floor_plane'), k_);
    back_plane  = Adsharp(cs.get_object_pose('back_plane'),  k_);
    ...
    while ~controller.system_reached_stable_region()
        ...
        q =  cs.get_joint_positions(jointnames);
        x_pose = robot.fkm(q);
        ...
        u_log(:,j) = u;
        q_dot(:,j) = cs.get_joint_velocities(jointnames);
        j = j+1;
    end
    cs.stop_simulation();
